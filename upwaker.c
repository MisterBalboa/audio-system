...
void upwaker_wake_up(upwaker_t *upwaker) {
  ...
  // Must call,
  // in case the other thread had finished
  // the 'if' test before this function was called.
  // (i.e. it is not safe to trylock the mutex
  // to check wether to call pthread_cond_broadcast).
}
